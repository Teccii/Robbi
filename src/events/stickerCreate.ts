...
const stickerCreate: Event = {
    ...
    exec: async (client, sticker: Sticker) => {
        ...
        */
        ...
        const logChannelIds = sticker.settings.events.filter(v => v.event == "expressionCreate").map(v => v.channel);
            ...
            if (logChannel && !logChannel.isDMBased() && logChannel.isTextBased() && logChannel.id != sticker.id) {
                await logChannel.send({
                    embeds: [client.simpleEmbed({
                        title: `Sticker :${sticker.name}: created`,
