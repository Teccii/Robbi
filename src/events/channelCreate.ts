...
const channelCreate: Event = {
    ...
    exec: async (client, channel: GuildChannel) => {
        ...
        */
        ...
        const logChannelIds = channel.settings.events.filter(v => v.event == "channelCreate").map(v => v.channel);
            ...
            if (logChannel && !logChannel.isDMBased() && logChannel.isTextBased() && logChannel.id != channel.id) {
                ...
                switch (channel.type) {
                    case ChannelType.GuildText: {
                        title = `Text channel "${channel.name}" created`;
                        break;
                    }

                    case ChannelType.GuildVoice: {
                        title = `Voice channel "${channel.name}" created`;
                        break;
                    }

                    case ChannelType.GuildForum: {
                        title = `Forum channel "${channel.name}" created`;
                        break;
                    }

                    case ChannelType.GuildAnnouncement: {
                        title = `News channel "${channel.name}" created`;
                        break;
                    }

                    case ChannelType.GuildCategory: {
                        title = `Category "${channel.name}" created`;
                        break;
                    }
                }
